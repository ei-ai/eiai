...
| (2) 전체 시스템 구성 | 다음은 본 연구에서 사용하고자 하는 NAS 알고리즘을 도식화한 것이다. <br/> <br> &nbsp; __1.	Search space__ <br> Search space는 알고리즘이 탐색을 수행하는 공간을 의미한다. NAS는 이 공간 내에서 최적의 모델 아키텍처를 탐색한다. 탐색 공간은 탐색할 수 있는 모든 모델 구조들의 집합으로, 어떤 연산을 쓸지, 연산들이 어떻게 연결될지 등이 정의되어 있다. 본 연구에서는 다음과 같은 구성 요소를 포함한 탐색 공간을 정의한다: <br> -	Arbitrary Encoder-Decoder Attention <br> 모든 encoder layer 를 decoder가 자유롭게 참조할 수 있도록 한다. <br> -	Heterogeneous Transformer Layers <br> 각 layer 별로 hidden size, head 수, FFN 크기 등이 다르게 구성될 수 있어, 다양한 하드웨어 특성에 맞춘 유연한 아키텍처 설계가 가능하다. <br> -	Hyperparameter (QKV Dimension) <br> 각 하드웨어에 최적화된 qkv dim을 조정하도록 구현하면서 메모리 사이즈를 고려한 트랜스포머의 탐색이 가능하도록 한다.<br><br> &nbsp; __2.	Search Strategy__ <br> Search Strategy는 탐색 방법으로 Search space 내의 어떤 아키텍처를 선택할지 결정하는 전략이다. 탐색 방법으로는 강화학습, 진화 알고리즘, 그래디언트 방식 등 여러 알고리즘이 쓰인다. 본 연구에서는 Supertransformer라는 모든 Subtransformer들을 포함하는 큰 네트워크를 한 번 훈련시키고, 진화 알고리즘(Evolutionary Search)를 통해 하드웨어 latency가 가장 낮고 성능이 좋은 Subtransformer를 탐색한다. 또한, HPO(Hyperparameter Optimization)을 통해 하드웨어에 최적화된 qkv dim 구성을 선택할 수 있도록 한다.<br><br> &nbsp; __3.	Performance Estimation Strategy__ <br> Performance Estimation Strategy는 선택된 아키텍처의 성능을 평가하거나 추정하는 방법이다. 보통 NAS는 search strategy가 하나 혹은 여러 개의 후보 아키텍처를 추출하고, 이들의 성능을 예측한다. 본 연구에서는 다음과 같은 성능 추정 방법을 채택한다: <br> -	weight sharing <br> subtransformer의 개별 학습 없이 이미 학습된 supertransformer의 weight를 사용하여 빠른 성능 평가가 가능한다. <br> -	하드웨어 기반 Latency 측정<br> FLOPs 대신 실제 하드웨어에서의 지연시간을 측정하여 플랫폼 맞춤 성능 평가가 가능하다. <br><br>✅ __실험 환경 구성__ <br> - 보드: ROCK Pi 5B (Rockchip RK3588 ARM SoC 기반 NPU 보드)<br> - 운영체제 (OS): Ubuntu 22.04 (aarch64), Debian (Radxa Rock 5B 이미지)<br> - 모델 아키텍처: Transformer 기반 구조 <br> - 사용 데이터셋: WMT’14 En-De, WMT’14 En-Fr, WMT’19 En-De, IWSLT’14 De-En<br> <details> <summary>파이썬 라이브러리 목록</summary> rknn-toolkit2==2.3.0 <br> rknn-toolkit-lite2==1.5.0  <br> charset-normalizer==3.4.0  <br> datasets==3.1.0  <br> fast-histogram==0.13<br>   flatbuffers==24.3.25<br> huggingface-hub==0.26.5<br> mpmath==1.3.0<br> multidict==6.1.0<br> multiprocess==0.70.16<br> numpy==1.24.4<br> onnx==1.14.1 <br> onnxoptimizer==0.2.7 <br> onnxruntime==1.16.0 <br> opencv-python==4.10.0.84<br>packaging==24.2<br> pandas==2.0.3<br> safetensors==0.4.5<br>  scipy==1.10.1<br> sympy==1.13.3 <br> tokenizers==0.20.3<br> torch==2.4.1<br> torchdata==0.7.1<br> torchtext==0.15.2<br> torchvision==0.19.1<br> tqdm==4.67.1<br> transformers==4.46.3 </details>|
| (3) 주요엔진 및 기능 설계 | 본 연구의 프레임워크는 다음과 같다:
1. 모든 SubTransformer들의 조합을 포함한 거대한 모델인 **SuperTransformer**를 학습시킨다.  
2. 특정 하드웨어 플랫폼에 대해 **(SubTransformer 아키텍처, 측정된 latency)** 쌍으로 구성된 데이터셋을 구축한다.  
3. 위의 데이터셋을 통해 **latency predictor**를 학습시킨다.  
4. 학습된 latency predictor를 사용하여 하드웨어의 **Latency 제약 조건** 하에서 **evolutionary search**를 수행하고, 해당 하드웨어에 최적화된 SubTransformer 모델을 탐색한다.  
5. 최종 선발된 SubTransformer 모델을 **다시 학습**하여 최종 성능을 도출한다.  

## ✅ NAS 기반 상세 구현
### 1. Search Space
**SuperTransformer**는 search space에서 가장 큰 모델로 정의되며, 이를 통해 다양한 **SubTransformer**를 랜덤 샘플링하여 학습한다. 구성 요소는 다음과 같다:
- **Arbitrary Encoder-Decoder Attention**  
  디코더의 각 레이어가 인코더의 다양한 레이어를 선택적으로 참조할 수 있도록 구현된다.  
  여러 인코더 레이어의 key/value를 concat하여 디코더에 전달하며, 이는 거의 추가 비용 없이 정확도를 향상시킨다.
- **Heterogeneous Transformer Layers**  
  각 레이어마다 Embedding Dim, FFN Hidden Dim, Head 수, Layer 수 등을 유동적으로 설정할 수 있어, 다양한 하드웨어에 적합한 구조로 조정 가능하다.
- **Hyperparameter (QKV Dimension)**  
  Q, K, V의 차원을 탐색 공간에 포함시켜 memory usage 및 연산량을 조절할 수 있으며,  
  이를 통해 다양한 플랫폼 환경에서 latency와 성능을 동시에 만족하는 구조를 찾을 수 있다.

### 2. Search Strategy
- **Evolutionary Search**  
  학습된 latency predictor를 활용해 다양한 SubTransformer 후보 아키텍처의 latency를 예측하고,  
  latency 제약을 만족하면서 validation loss가 가장 낮은 아키텍처를 탐색하는 방식이다.
- **HPO (Hyperparameter Optimization)**  
  탐색 공간에 포함된 qkv dim과 같은 하이퍼파라미터에 대해 HPO를 적용함으로써  
  latency와 성능의 균형을 만족하는 최적 조합을 찾는다.


### 3. Performance Estimation Strategy

- **Weight Sharing**  
  SubTransformer는 SuperTransformer의 weight를 공유하여 학습 비용을 절감한다.  
  임베딩, FFN, Attention, 레이어 수 등 다양한 구성에서 필요한 부분만 슬라이싱하여 사용하며  
  별도의 학습 없이 효율적으로 평가 가능하다.

- **Latency Predictor 기반 예측**  
  하드웨어에서 실제 측정된 latency 값을 기반으로 predictor를 학습시킨다.  
  탐색 중에는 하드웨어 latency를 직접 측정하지 않고도 정확한 latency 추정이 가능하다. |
